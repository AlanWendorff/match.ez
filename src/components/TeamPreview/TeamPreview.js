...
const TeamPreview = ({ teamid, color, matches, prevMatch, setVs, setHistory, roster, winRate, winStrike, wl, img }) => {
    ...
    const [LASTMATCH] = useState(prevMatch[0]);
    const [NEXTMATCH] = useState(matches[0]);

    const arrayTeam =
        prevMatch[0] && prevMatch[0].opponents.find((element) => element.opponent.id === parseInt(teamid));
    const rankingTeam =
        prevMatch[0] && ranking.find((element) => element.name.toLowerCase() === arrayTeam.opponent.name.toLowerCase());
    const getPlayerInfo = (playerName, LASTNAME) => {
        ...
        axios.get(PLAYER_INFO.replace(":name", playerName), config).then(({ data }) => {
            data.name.includes(LASTNAME) ? setPlayerInfo(data) : setPlayerInfo(false);
        });
    };
                    ...
                    {prevMatch !== "no-match" ? (
                        <>
                            ...
                            <div>
                                ...
                                <span
                                    className={
                                        LASTMATCH.results[0].score > LASTMATCH.results[1].score ? "winner-dot" : ""
                                    }
                                    style={{
                                        backgroundColor:
                    ...
                    {roster.map((player) => {
                        ...
                        return (
                            ...
                            >
                                ...
                                <span style={{ color: color.DarkVibrant }} className="player-name-style">
                                    {FIRSTNAME + " " + LASTNAME}
                                </span>
                            </div>
                        );
