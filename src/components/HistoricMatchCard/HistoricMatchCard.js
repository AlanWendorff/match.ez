...
const HistoricMatchCard = ({ match, teamId }) => {
    const history = useHistory();
    const [loading, setLoading] = useState(false);
    const [content, setContent] = useState(false);
    const [showscore, setShowScore] = useState(0);
    const [playerscore, setPlayerScore] = useState([]);
    const { detailed_stats, bestOf, league, serie, begin_at, id, opponents, results, stage } = match;
    ...
    `;
    const Twitter = `${opponents[0].opponent.name}: ${results[0].score} VS ${opponents[1].opponent.name}: ${
        results[1].score
    } | ${league.name + " " + serie.full_name} -> ${window.location.href}`;
    const Wapp = `${opponents[0].opponent.name}: ${results[0].score} VS ${opponents[1].opponent.name}: ${
        results[1].score
    } |  ${league.name + " " + serie.full_name} -> ${window.location.href}`;
        ...
        >
            <div className="prev-game-content cursor-default">
                <div className="prev-game-header-container">
                    <p className="text-align-center" style={{ color: colorLeague.DarkVibrant }}>
                        {stage}
                    </p>
                </div>
                ...
                <div className="prev-game-mobile">
                    <div
                        ...
                        onClick={() => {
                            history.push(TEAM.replace(":teamid", opponents[0].opponent.id));
                        }}
                        style={{
                            borderLeft:
                                results[1].score < results[0].score &&
                                `5px solid ${opponents[0].opponent.colors.DarkVibrant}`,
                            borderRight:
                                results[1].score < results[0].score &&
                                `5px solid ${opponents[0].opponent.colors.DarkVibrant}`,
                        }}
                    >
                        <div className={results[0].score < results[1].score ? "match-loser" : "match-winner"}>
                            <ProgressiveImage
                                src={opponents[0].opponent.image_url === null ? nopic : opponents[0].opponent.image_url}
                                placeholder={loader}
                            >
                        ...
                        <p
                            className={results[0].score < results[1].score ? "match-loser" : "match-winner"}
                            style={{
                                backgroundColor: results[0].score > results[1].score && colorTeamA.DarkVibrant,
                    ...
                    <div
                        ...
                        onClick={() => {
                            history.push(TEAM.replace(":teamid", opponents[1].opponent.id));
                        }}
                        style={{
                            borderLeft:
                                results[0].score < results[1].score &&
                                `5px solid ${opponents[1].opponent.colors.DarkVibrant}`,
                            borderRight:
                                results[0].score < results[1].score &&
                                `5px solid ${opponents[1].opponent.colors.DarkVibrant}`,
                        }}
                    >
                        <div className={results[0].score < results[1].score ? "match-winner" : "match-loser"}>
                            <ProgressiveImage
                                src={opponents[1].opponent.image_url === null ? nopic : opponents[1].opponent.image_url}
                                placeholder={loader}
                            >
                        ...
                        <p
                            className={results[0].score < results[1].score ? "match-winner" : "match-loser"}
                            style={{
                                backgroundColor: results[0].score < results[1].score && colorTeamB.DarkVibrant,
                                color: results[0].score < results[1].score && "white",
                            }}
                        >
            ...
            {content && (
                <>
                    {detailed_stats === true ? (
                        showscore === 0 ? (
                            <div
                                className="player-stats-btn highlight-text"
                                onClick={() => {
                                    playerScore();
                                    setShowScore(1);
                                }}
                    ...
                    <p className="info-container">
                        <span className="label-data-style" style={{ color: palette.DarkVibrant }}>
                            <FontAwesomeIcon icon={faTrophy} />
                        </span>
                        <span className="text-align-end">{league.name + " " + serie.full_name}</span>
                    </p>
                    <p className="info-container">
                        <span className="label-data-style" style={{ color: palette.DarkVibrant }}>
